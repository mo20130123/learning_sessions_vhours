<style>
 ...
 <template>
  <div class="page-wrapper">
     ...
     <h2 class="font-italic font-weight-light font-weight-bold">Recipe Edit</h2>

     <router-link to="/Recipe"> <span> Recipes </span> </router-link> / <span>Edit</span>
     <br><br>
...
<v-card>
   ...
   <v-container>
    <v-row>
     <v-col>
          <v-select  outlined v-model="Ef.recipes_category_ids" :items="Categories" item-text="label" item-value="value" label="Category" multiple></v-select>
          ...
          <v-text-field label="time En" v-model="Ef.time_en" outlined :rules="requiredRules"></v-text-field>
          <v-text-field label="time Ar" v-model="Ef.time_ar" outlined :rules="requiredRules"></v-text-field>
          <v-text-field label="servings En" v-model="Ef.servings_en" outlined :rules="requiredRules"></v-text-field>
          <v-text-field label="servings Ar" v-model="Ef.servings_ar" outlined :rules="requiredRules"></v-text-field>
     ...
     <v-col>
          ...
          <v-textarea label="Description En" v-model="Ef.body_en" name="input-7-4" outlined rows="8" :rules="requiredRules"></v-textarea>
          <v-textarea label="Description Ar" v-model="Ef.body_ar" name="input-7-4" outlined rows="8" :rules="requiredRules"></v-textarea>
         ...
         <tbody is="transition-group" name="my-list">
            <tr v-for="(image,index) in images" :key="image.cc" >
               <td width="40%">
                   ...
                   <v-file-input v-if="!image.id" :rules="requiredRules" @change="Preview_image($event,image)" name="images[]" :label="'Image '+(1+index)" hint="220*220 Pixels" :persistent-hint="true" prepend-icon="" show-size outlined  ></v-file-input>
               </td>
      ...
      <table class="table">
         ...
         <tbody is="transition-group" name="my-list" id="tbody_add_keyword">
            <tr v-for="(Ingredient,index) in Ingredients" :key="Ingredient.cc" >
               <td width="30%">
                   <input type="hidden" v-model="Ingredient.id" name="old_ingredient_ids[]" >
               ...
               <td width="12%">
                 <v-text-field label="Quantity" v-model="Ingredient.quantity" outlined :rules="requiredRules" ></v-text-field>
               </td>
               <td>
...
<v-row>
   ...
   <v-col>
      <v-btn large color="success" width="100%" @click="edit()" :disabled="btn_submit" >Update Recipe</v-btn>
   </v-col>
</v-row>
       ...
       getProductById(BProduct)
       {
        ...
        getCategories_list()
        {
         ...
         getResults(){
              ...
              $.ajax({
                    type:"get",
                    url: `${Admin_BASE_url}/Recipe/show/${this.$route.params.id}` ,
                    headers: { 'jwt': localStorage.getItem('jwt') },
               })
               .done(function(Response){
                  if(Response.status == 'unValidJWT'){
                  ...
                  this.Ef = Response.Recipe;
                  this.Ef.body_ar = Response.Recipe.body_ar.replaceAll('\\n','\n') ;
                  this.Ef.body_en = Response.Recipe.body_en.replaceAll('\\n','\n') ;
