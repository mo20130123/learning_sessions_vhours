<style>
 ...
 <template>
  <div class="page-wrapper">
     ...
     <h2 class="font-italic font-weight-light font-weight-bold">Bundel Edit</h2>

     <router-link to="/Bundel"> <span> Bundels </span> </router-link> / <span>Edit</span>
     <br><br>
   ...
   <v-container>
      <v-row>
         <v-col>
               ...
               <v-text-field label="Bundle Label En" v-model="Ef.bundle_summary_en" outlined :rules="requiredRules"></v-text-field>
               <v-text-field label="Bundle Label Ar" v-model="Ef.bundle_summary_ar" outlined :rules="requiredRules"></v-text-field>
               <v-text-field label="Quantity in Stock :" type="number" v-model="Ef.quantity" outlined  :rules="requiredRules"></v-text-field>
               <v-text-field label="SAP code :" v-model="Ef.sap_code" outlined  ></v-text-field>
               <v-text-field label="Main Price (EGP):" v-model="Ef.price" type="number" outlined :rules="requiredRules"></v-text-field>
           </v-col>
          <v-col>
               ...
               <v-text-field label="Short Description En" v-model="Ef.short_description_en" outlined :rules="requiredRules"></v-text-field>
               <v-text-field label="Short Description Ar" v-model="Ef.short_description_ar" outlined :rules="requiredRules"></v-text-field>
       ...
       <table class="table">
          ...
          <tbody is="transition-group" name="my-list" id="tbody_add_keyword">
             <tr v-for="(Keyword,index) in Keywords" :key="Keyword.cc" >
                <td>
                    <input type="hidden" name="Keywords_id[]" :value="Keyword.id" >
...
<v-row>
   ...
   <v-col>
      <v-btn large color="success" width="100%" @click="edit()" :disabled="btn_submit" >Update Bundel</v-btn>
   </v-col>
</v-row>
...
<script>
  ...
  export default {
    name: 'Bundel-edit',
    components: {spinner3,spinner2 },
     data: () => ({
        Permission: 'Bundel',
        api_update: 'Bundel/update',
        show_spinner:false,
        btn_submit:false,
        Ef: {
             is_main: null,
    ...
    methods:{
         getResults(){
              ...
              $.ajax({
                    type:"get",
                    url: `${Admin_BASE_url}/Bundel/show/${this.$route.params.id}` ,
               ...
               .done(function(Response){
                  ...
                  this.Ef = Response.Bundel ;
                  this.Ef.description_en = Response.Bundel.description_en.replaceAll('\\n','\n') ;
                  this.Ef.description_ar = Response.Bundel.description_ar.replaceAll('\\n','\n') ;
                  this.BundleProducts = Response.BundleProducts;
    ...
    computed: {
        subCat()
        {
              var get_subCat = this.CatsSubCats.find(obj=>obj.value == this.Ef.category_id);
              if(get_subCat){
                 return get_subCat.subCategoiry;
              }
        },
        new_price()
        {
            if(this.Ef.old_price === "" || this.Ef.discount_percentage === "")
            {
                ...
                let discount = this.Ef.old_price * (this.Ef.discount_percentage / 100); console.log( this.Ef.price = Math.round(this.Ef.old_price - discount));
