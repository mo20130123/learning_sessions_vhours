<template>
  <div class="page-wrapper">
     ...
     <h2 class="font-italic font-weight-light font-weight-bold">Product Sub Categories</h2>
     ...
     <section class="section-page-control">
        <v-row>
           <v-col cols="5" >
             ...
             <router-link to="/ProductSubCategory/sort"> <v-btn class="ma-2" outlined color="black"> <b-icon-filter></b-icon-filter> Sort  </v-btn> </router-link>

          </v-col>
          <v-col>
                <div class="text-right">
                   <v-select @change="getResults()" outlined v-model="ST.category_id" :items="ProductCategory" item-text="label" item-value="value" label="Category" style="width:42%;display:inline-block;margin-right:10px"  ></v-select>
              ...
              <v-card-text>
                <v-container>
                  <v-form  id="create_form" ref="createForm" v-model="isValid_createForm" :lazy-validation="false" >
                      <v-autocomplete outlined v-model="CF.category_id" :items="ProductCategory" item-text="label" item-value="value" label="Category" :rules="requiredRules" ></v-autocomplete>
              ...
              <v-card-text>
                <v-container>
                  <v-form  id="edit_image" ref="editForm" v-model="isValid_editForm" :lazy-validation="false" >
                      <v-autocomplete outlined v-model="EF.category_id" :items="ProductCategory" item-text="label" item-value="value" label="Category" :rules="requiredRules" ></v-autocomplete>
...
<script>
  ...
  export default {
    ...
    data: () => ({
        Permission: 'Product',
        api_get_list: 'ProductSubCategory/list',
        api_showORhide: 'ProductSubCategory/showORhide',
        api_delete: 'ProductSubCategory/delete',
        api_create: 'ProductSubCategory/create',
        api_update: 'ProductSubCategory/update',
        ProductCategory: []
    }),//end data
    created(){
    ...
    methods:{
      getCategories_list()
      {
            $.ajax({
               type:"get",
               url: `${Admin_BASE_url}/ProductSubCategory/selection_data` ,
           ...
           .done(function(Response){
              ...
              this.ProductCategory = Response.ProductCategories;
              this.ProductCategory.unshift({ label:'All ' , value: null });
           }.bind(this))
           .fail(function(data){
