<template>
  <div class="page-wrapper">
     ...
     <h2 class="font-italic font-weight-light font-weight-bold">District</h2>
     ...
     <section class="section-page-control">
        <v-row>
           <v-col cols="4" >
             ...
             <!-- <router-link to="/District/sort"> <v-btn class="ma-2" outlined color="black"> <b-icon-filter></b-icon-filter> Sort  </v-btn> </router-link> -->

          </v-col>
          <v-col>
              <div class="text-right">
                 <v-select @change="getResults()" outlined v-model="ST.city_id" :items="Cities" item-text="label" item-value="value" label="Cities" style="width:42%;display:inline-block;margin-right:10px"  ></v-select>
              ...
              <v-card-text>
                <v-container>
                  <v-form  id="create_form" ref="createForm" v-model="isValid_createForm" :lazy-validation="false" >
                      <v-row>
                        <v-col>
                             <v-select  outlined v-model="CF.city_id" :items="Cities" item-text="label" item-value="value" label="Cities" :rules="requiredRules" ></v-select>
              ...
              <v-card-text>
                <v-container>
                  <v-form  id="edit_image" ref="editForm" v-model="isValid_editForm" :lazy-validation="false" >
                      <v-row>
                        <v-col>
                            <v-select  outlined v-model="EF.city_id" :items="Cities" item-text="label" item-value="value" label="Cities" :rules="requiredRules" ></v-select>
...
<script>
  ...
  export default {
    ...
    mixins: [AdminMixin],
     data: () => ({
        Permission: 'District',
        api_get_list: 'District/list',
        api_showORhide: 'District/showORhide',
        api_delete: 'District/delete',
        api_create: 'District/create',
        api_update: 'District/update',
        Cities: []
    }),//end data
    created(){
      this.getCities_list();
