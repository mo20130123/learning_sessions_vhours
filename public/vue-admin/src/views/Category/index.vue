...
<template>
  <div class="page-wrapper">
     ...
     <h2 class="font-italic font-weight-light font-weight-bold">Categories ({{ mainList.total }})</h2>
     ...
     <section class="section-page-control">
        <v-row>
           <v-col cols="7" >
             ...
             <router-link to="/Category/sort"> <v-btn class="ma-2" outlined color="black"> <b-icon-filter></b-icon-filter> Sort  </v-btn> </router-link>
                ...
                <v-container>
                  <v-form  id="create_form" ref="createForm" v-model="isValid_createForm" :lazy-validation="false" >
                        ...
                        <div class="div_Preview_image">
                            <img :src="CF.banner" id="Preview_image_create" class="Preview_image" >
                            <v-file-input @change="Preview_image($event,'Preview_image_create')" label="Banner" name="banner" hint="1360*212 Pixels" :persistent-hint="true" prepend-icon="" show-size outlined ></v-file-input>
                        </div>
                ...
                <v-container>
                  <v-form  id="edit_image" ref="editForm" v-model="isValid_editForm" :lazy-validation="false" >
                        ...
                        <div class="div_Preview_image">
                            ...
                            <v-file-input @change="Preview_image($event,'Preview_image_edit')" label="Banner" name="banner" hint="1360*212 Pixels" :persistent-hint="true" prepend-icon="" show-size outlined ></v-file-input>
                        </div>
...
<script>
  ...
  export default {
    ...
    mixins: [AdminMixin],
     data: () => ({
        Permission: 'Category',
        api_get_list: 'Category/list',
        api_showORhide: 'Category/showORhide',
        api_delete: 'Category/delete',
        api_create: 'Category/create',
        api_update: 'Category/update',
        CF: {
            banner: ''
        }
    }),//end data
    ...
    methods:{
    //    Preview_banner(e,from_id)
    //    {  console.log('Preview_image => '+from_id);
    //      if(e) {
    //         $('#'+from_id).attr('src', URL.createObjectURL(e));
    //      }
    //    },
    //    Preview_banner(e,item)
    //    {
    //         if(e) {
    //             item.banner = URL.createObjectURL(e);
    //         }
    //    },
    }//End methods
}//End export default
